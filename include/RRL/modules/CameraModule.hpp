...
public:
     ...
     * @param view_basis specifies the camera coordinate system. By default it uses the OpenGL standard.
    ...
    ObjectID SpawnCamera(
        ...
        rrl::camera::CameraViewBasis view_basis = rrl::camera::CameraViewBasis::STANDARD_OPENGL,
    ...
    /**
     * @brief Resets the camera to use the global scene environment (Default).
     */
    void SetCameraBackgroundToScene(ObjectID cam_obj);
    /**
     * @brief Overrides the background for this camera with a custom solid color.
     */
    void SetCameraBackgroundColor(ObjectID cam_obj, const glm::vec4& color);
    /**
     * @brief Overrides the background for this camera with a flat 2D texture.
     * Useful for Augmented Reality video passthrough feeds.
     */
    void SetCameraBackgroundFlatTexture(ObjectID cam_obj, rrl::AssetID texture_asset);
