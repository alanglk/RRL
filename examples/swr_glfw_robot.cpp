...
public:
    ...
    void Initialize(rrl::Engine& engine, 
        ...
        rrl::rhi::RHIRenderTargetDescriptor rt_desc;
        rt_desc.width = robot_cam_width;
        rt_desc.height = robot_cam_height;

        // Color attachment descriptor
        rrl::rhi::RHIRenderAttachmentDescriptor color_attach;
        color_attach.id = "robot_cam_color";
        color_attach.data_type = rrl::asset::ImageAssetType::UINT8;
        color_attach.channels = rrl::asset::ImageChannelLayout::CH_3;
        rt_desc.color_attachments.push_back(color_attach);
        ...
        robot_cam = engine.camera.SpawnCamera(camera_model, rrl::camera::CameraViewBasis::STANDARD_OPENGL, cam_target, rrl::rhi::RHIRenderLayerMask::LAYER_DEFAULT);
    ...
    void Update(rrl::Engine& engine) {
        if (cam_target != rrl::rhi::RESOURCE_NULL) {
            // Read back pixels from the COLOR ATTACHMENT, not the FBO itself
...
int main() {
    ...
    engine.camera.SetCameraLayer(main_camera, 
        rrl::rhi::RHIRenderLayerMask::LAYER_DEFAULT | rrl::rhi::RHIRenderLayerMask::LAYER_DEBUG | rrl::rhi::RHIRenderLayerMask::LAYER_UI
    );
