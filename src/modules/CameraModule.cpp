...
ObjectID CameraModule::SpawnCamera(
    ...
    rrl::camera::CameraViewBasis view_basis,
    rrl::rhi::ResourceID target_fbo,
    rrl::rhi::RHIRenderLayerMask layer
) {
    return ToObjectID( rrl::camera::SpawnCamera(m_ctx->registry, model, view_basis, target_fbo, layer) );
}
...
void CameraModule::SetCameraBackgroundToScene(ObjectID cam_obj){
    rrl::camera::SetCameraBackgroundToScene(m_ctx->registry, ToEntt(cam_obj));
}
void CameraModule::SetCameraBackgroundColor(ObjectID cam_obj, const glm::vec4& color) {
    rrl::camera::SetCameraBackgroundColor(m_ctx->registry, ToEntt(cam_obj), color);
}
void CameraModule::SetCameraBackgroundFlatTexture(ObjectID cam_obj, rrl::AssetID texture_asset) {
    rrl::camera::SetCameraBackgroundFlatTexture(m_ctx->registry, ToEntt(cam_obj), texture_asset);
}
