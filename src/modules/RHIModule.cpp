...
rrl::asset::ImageAsset RHIModule::GetTargetImage(rrl::rhi::ResourceID id, rrl::rhi::RHIRenderAttachmentSemantic semantic, uint32_t array_index) {
    return rrl::rhi::GetTargetImage(m_ctx->registry, id, semantic, array_index);
}
void RHIModule::SetPresentationTarget(rrl::rhi::ResourceID id, rrl::rhi::RHIRenderAttachmentSemantic semantic, uint32_t array_index) {
    rrl::rhi::SetPresentationTarget(m_ctx->registry, id, semantic, array_index);
}


// --- Render Targets (FBOs) ---------------------------------------
void RHIModule::AllocateRenderTargetTexture(rrl::rhi::ResourceID id, uint32_t width, uint32_t height, 
                           rrl::asset::ImageAssetType data_type, rrl::asset::ImageChannelLayout channels, 
                           bool is_depth, uint32_t array_layers) {
    rrl::rhi::AllocateRenderTargetTexture(m_ctx->registry, id, width, height, data_type, channels, is_depth, array_layers);
}

void RHIModule::CreateRenderTarget(rrl::rhi::ResourceID id, const rrl::rhi::RHIRenderTargetDescriptor& desc) {
...
void RHIModule::DestroyRenderTargetTexture(rrl::rhi::ResourceID id) {
    rrl::rhi::DestroyRenderTargetTexture(m_ctx->registry, id);
}
