...
rrl::asset::ImageAsset RHIModule::GetTargetImage(rrl::rhi::RenderTargetHandle handle) {
    return rrl::rhi::GetTargetImage(m_ctx->registry, handle);
}


// --- Render Targets (FBOs) ---------------------------------------
rrl::rhi::RenderTargetHandle RHIModule::CreateRenderTarget(uint32_t width, uint32_t height) {
    return rrl::rhi::CreateRenderTarget(m_ctx->registry, width, height);
}

void RHIModule::DestroyRenderTarget(rrl::rhi::RenderTargetHandle handle) {
    rrl::rhi::DestroyRenderTarget(m_ctx->registry, handle);
}
