...
// --- PCD Prefab Adapter ------------------------------------------
static IOPrefab LoadPcdPrefab(const std::string& filepath) {
    IOPrefab scene;
    scene.filepath = filepath;

    asset::MeshAsset pcd_mesh = LoadPCDFile(filepath);
    if (pcd_mesh.positions.empty()) {
        LOG_ERROR("[PrefabIO] Failed to load PCD mesh or file is empty: {}", filepath);
        return scene;
    }

    IONode node;
    node.name = fs::path(filepath).stem().string();
    std::transform(node.name.begin(), node.name.end(), node.name.begin(), ::tolower);
    std::replace(node.name.begin(), node.name.end(), '.', '_');

    node.mesh = std::move(pcd_mesh);
...
IOPrefab LoadPrefab(const std::string& filepath) {
    ...
    if (ext == ".pcd") return LoadPcdPrefab(filepath);
