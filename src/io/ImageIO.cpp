...
// --- I/O ---------------------------------------------------------
ImageAsset LoadImage(const std::string& filepath) {
    ImageAsset img;
    ...
    if (stbi_is_hdr(filepath.c_str())) {
        ...
        img.data_type = ImageAssetType::FLOAT32;
        ...
        img.data_type = ImageAssetType::UINT16;
        ...
        img.data_type = ImageAssetType::UINT8;
...
ImageAsset LoadImage(const uint8_t* file_data, size_t data_size) {
    ImageAsset img;
    int width = 0, height = 0, channels = 0;
...
bool SaveImage(const std::string& filepath, const ImageAsset& image) {
    if (image.data.empty()) {
    ...
    int stride = image.width * ch * ((image.data_type == ImageAssetType::FLOAT32) ? 4 : (image.data_type == ImageAssetType::UINT16) ? 2 : 1);
    ...
    else if (ext == ".hdr" && image.data_type == ImageAssetType::FLOAT32) {
