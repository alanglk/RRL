...
entt::entity SpawnCameraFrustum(entt::registry& registry, entt::entity camera_entity, float visual_length) {
    ...
    rrl::asset::BindMesh(registry, frustum_entity, mesh_asset, { debug_mat }, rrl::rhi::RHIRenderLayerMask::LAYER_DEBUG);
...
entt::entity SpawnDebugGrid(entt::registry& registry, float size, int subdivisions) {
    ...
    // Bind to the debug layer
    rrl::asset::BindMesh(registry, grid_entity, mesh_asset, { debug_mat }, rrl::rhi::RHIRenderLayerMask::LAYER_DEBUG);
    return grid_entity;
}
