// RRL/src/scene/PrefabManager.cpp
...
static const PrefabNodeBlueprint* ResolveBlueprintPath(const PrefabCache& cache, const PrefabID& node_id_path) {
...
entt::entity SpawnPrefab(entt::registry& registry, const PrefabID& node_id_path) {
    RRL_ASSERT(registry.ctx().contains<PrefabCache>(), "SceneManager not initialized!");
    auto& cache = registry.ctx().get<PrefabCache>();
