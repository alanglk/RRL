...
void SWRDrawPoint(
    rrl::asset::ImageAsset& rt, std::vector<float>& depth_buffer, 
    const glm::vec3& p, int radius, const glm::vec3& color, const ColorFormatCache& format) 
{
    ...
    for (int y = min_y; y <= max_y; ++y) {
        ...
        for (int x = min_x; x <= max_x; ++x) {
            int dx = x - cx;
            if (dx * dx + dy2 <= r2) {
                size_t depth_idx = y * rt.width + x;
                if (z <= depth_buffer[depth_idx]) {
...
void SWRDrawLine(
    rrl::asset::ImageAsset& render_target, std::vector<float>& depth_buffer, 
    ...
    const glm::vec3& color, const ColorFormatCache& format) 
{
    ...
    if (dist == 0) {
        if (x0 >= 0 && x0 < render_target.width && y0 >= 0 && y0 < render_target.height) {
            size_t depth_idx = y0 * render_target.width + x0;
            if (z0 <= depth_buffer[depth_idx]) {
                depth_buffer[depth_idx] = z0;
    ...
    for (int i = 0; i <= dist; i++) {
        if (x0 >= 0 && x0 < render_target.width && y0 >= 0 && y0 < render_target.height) {
            ...
            size_t depth_idx = y0 * render_target.width + x0;
            if (z - 0.0005f <= depth_buffer[depth_idx]) {
                depth_buffer[depth_idx] = z; 
                SetPixel(render_target, x0, y0, r, g, b, format);
            }
        }
...
void RasterizeTriangle(
    ...
    for (int y = min_y; y <= max_y; ++y) {
        for (int x = min_x; x <= max_x; ++x) {
            ...
            size_t depth_idx = y * render_target.width + x;
            float interpolated_z = w0 * v0.z + w1 * v1.z + w2 * v2.z;
            if (interpolated_z >= depth_buffer[depth_idx]) continue; // Occluded
            depth_buffer[depth_idx] = interpolated_z; // Write depth
...
void SWRRender3DMesh(
    ...
    RRL_ASSERT(depth_buffer.size() >= (render_target.width * render_target.height), "[SWRRender3DMesh] Depth buffer is smaller than the render target!");
