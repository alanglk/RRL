...
static void OnMaterialSourceDestroyed(entt::registry& registry, entt::entity entity) {
    ...
    DecrementAssetRef(registry, ToEntt(mat.albedo_map) );
    DecrementAssetRef(registry, ToEntt(mat.normal_map) );
    DecrementAssetRef(registry, ToEntt(mat.metallic_roughness_map) );
    DecrementAssetRef(registry, ToEntt(mat.emissive_map) );
}
...
entt::entity CreateTexture(entt::registry& registry, const TextureID& texture_id, ImageAsset&& image_data) {
    ...
    if (registry.ctx().contains<AssetCache>() && !texture_id.empty()) {
        ...
        source.image = std::make_shared<ImageAsset>(std::move(image_data)); 
        source.version = 1;
...
entt::entity CreateMesh(entt::registry& registry, const MeshID& mesh_id,MeshAsset&& mesh_data) {
    ...
    if (registry.ctx().contains<AssetCache>() && !mesh_id.empty()) {
        ...
        source.mesh = std::make_shared<MeshAsset>(std::move(mesh_data)); // Move the geometry vectors
        source.version = 1;
...
void BindMesh(entt::registry& registry, entt::entity world_object, entt::entity mesh_asset, const std::vector<entt::entity>& materials, rhi::RHIRenderLayer layer) {
    ...
    // Copy the old linkage data (to avoid dangling pointers)
    entt::entity old_mesh = entt::null;
    std::vector<entt::entity> old_materials;
    if (auto* old_link = registry.try_get<MeshLinkage>(world_object)) {
        old_mesh = old_link->mesh_asset;
        old_materials = old_link->materials;
    }

    // Increment new assets first 
    ...
    // Decrement old assets last
    if (old_mesh != entt::null) DecrementAssetRef(registry, old_mesh);
    for (entt::entity mat : old_materials) {
        if (mat != entt::null) DecrementAssetRef(registry, mat);
    }
...
entt::entity CreateMaterial(entt::registry& registry, const MaterialID& material_id, const MaterialAsset& mat_data) {
    ...
    if (registry.ctx().contains<AssetCache>()) {
        ...
        MaterialAsset clean_data = mat_data;
        ...
        if (registry.valid(ToEntt(mat_data.albedo_map) ))               BindMaterialTexture(registry, mat_entity, ToEntt(mat_data.albedo_map), MaterialTextureType::ALBEDO);
        if (registry.valid(ToEntt(mat_data.normal_map) ))               BindMaterialTexture(registry, mat_entity, ToEntt(mat_data.normal_map), MaterialTextureType::NORMAL_MAP);
        if (registry.valid(ToEntt(mat_data.metallic_roughness_map) ))   BindMaterialTexture(registry, mat_entity, ToEntt(mat_data.metallic_roughness_map), MaterialTextureType::METALLIC_ROUGHNESS_MAP);
        if (registry.valid(ToEntt(mat_data.emissive_map) ))             BindMaterialTexture(registry, mat_entity, ToEntt(mat_data.emissive_map), MaterialTextureType::EMISSIVE_MAP);
        return mat_entity;
    }
...
void UpdateMaterial(entt::registry& registry, entt::entity material_asset, const MaterialAsset& mat_data) {
    ...
    MaterialAsset next_mat = mat_data;
    ...
    if (old_mat.albedo_map != mat_data.albedo_map)                          BindMaterialTexture(registry, material_asset, ToEntt(mat_data.albedo_map), MaterialTextureType::ALBEDO);
    if (old_mat.normal_map != mat_data.normal_map)                          BindMaterialTexture(registry, material_asset, ToEntt(mat_data.normal_map), MaterialTextureType::NORMAL_MAP);
    if (old_mat.metallic_roughness_map != mat_data.metallic_roughness_map)  BindMaterialTexture(registry, material_asset, ToEntt(mat_data.metallic_roughness_map), MaterialTextureType::METALLIC_ROUGHNESS_MAP);
    if (old_mat.emissive_map != mat_data.emissive_map)                      BindMaterialTexture(registry, material_asset, ToEntt(mat_data.emissive_map), MaterialTextureType::EMISSIVE_MAP);
}
void BindMaterialTexture(entt::registry& registry, entt::entity material_asset, entt::entity texture_asset, 
    ...
    if (registry.all_of<MaterialSourceComponent>(material_asset)) {
        ...
        if (target_slot && ToEntt(*target_slot) != texture_asset) {
            DecrementAssetRef(registry, ToEntt(*target_slot));
            IncrementAssetRef(registry, texture_asset);
            *target_slot = ToAssetID(texture_asset);
